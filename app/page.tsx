        ...
        <section className="bg-[#f5f6f2] py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            <div className="grid gap-px overflow-hidden border border-[#102433]/15 bg-[#102433]/15 md:grid-cols-3">
              {[{icon: ScanSearch, title: 'See the signal', text: 'Define the AI-native ambition and vision for your organization — the distinctive capabilities that will keep it ahead of competitors.'}, {icon: Network, title: 'Build the system', text: 'Design the operating architecture, data foundation, and intelligence layer, then sequence a transition roadmap tailored to how your organization creates value.'}, {icon: Waypoints, title: 'Partner for change', text: 'Move from strategy to durable transformation with a trusted partner, applying practices shaped by the Bay Area, Stanford, and leading consulting and engineering firms.'}].map(({ icon: Icon, title, text }, index) => <article key={title} className="group bg-[#f5f6f2] p-8 transition hover:bg-[#e5eef0] lg:p-10"><div className="flex items-start justify-between"><Icon className="text-[#1b7180]" /><span className="text-xs text-[#52717b]">0{index + 1}</span></div><h3 className="mt-20 text-2xl font-medium tracking-[-0.03em]">{title}</h3><p className="mt-4 leading-relaxed text-[#52717b]">{text}</p><ChevronRight className="mt-8 text-[#1b7180] transition group-hover:translate-x-1" /></article>)}
            </div>
          </div>
        </section>
