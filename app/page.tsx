...
import { ArrowUpRight, ChevronRight, Landmark, Network, ScanSearch, Waypoints } from "lucide-react"
      ...
      <main>
            ...
            <div className="grid gap-10 md:grid-cols-[0.75fr_1.5fr] md:items-start">
              ...
              <div>
                ...
                <div className="mt-10 grid gap-8 border-t border-[#102433]/15 pt-6 md:grid-cols-3">
                  ...
                  <div><p className="text-sm font-semibold text-[#102433]">Operator-tested</p><p className="mt-2 text-sm leading-relaxed text-[#52717b]">Built through experience at McKinsey, Google, and Microsoft — bringing strategy, product, and engineering discipline to complex transformations.</p></div>
                  <div><p className="text-sm font-semibold text-[#102433]">Capital Market-connected</p><p className="mt-2 text-sm leading-relaxed text-[#52717b]">Connected to the Stanford LP and Allocators Series, with a direct understanding of the GPs, LPs, and institutions that allocate capital.</p></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="bg-[#f5f6f2] py-20 lg:py-28">
          <div className="mx-auto max-w-7xl px-6 lg:px-10">
            ...
            <div className="grid gap-px overflow-hidden border border-[#102433]/15 bg-[#102433]/15 md:grid-cols-2 xl:grid-cols-4">
              {[{icon: ScanSearch, title: 'See the signal', text: 'Define the AI-native ambition and vision for your organization while understanding how capital markets value performance, resilience, and future readiness — so transformation increases enterprise value.'}, {icon: Network, title: 'Build the system', text: 'Design the operating architecture, data foundation, and intelligence layer, then sequence a transition roadmap tailored to how your organization creates value.'}, {icon: Waypoints, title: 'Partner for change', text: 'Move from strategy to durable transformation with a trusted partner, applying practices shaped by the Bay Area, Stanford, and leading consulting and engineering firms.'}, {icon: Landmark, title: 'Connect to capital markets', text: 'AIx2 brings extensive experience with allocators and fund managers, including our Stanford LP Series. We shape transformation around the requirements of capital formation and the expectations of the market.'}].map(({ icon: Icon, title, text }, index) => <article key={title} className="group bg-[#f5f6f2] p-8 transition hover:bg-[#e5eef0] lg:p-10"><div className="flex items-start justify-between"><Icon className="text-[#1b7180]" /><span className="text-xs text-[#52717b]">0{index + 1}</span></div><h3 className="mt-20 text-2xl font-medium tracking-[-0.03em]">{title}</h3><p className="mt-4 leading-relaxed text-[#52717b]">{text}</p><ChevronRight className="mt-8 text-[#1b7180] transition group-hover:translate-x-1" /></article>)}
            </div>
          </div>
        </section>
